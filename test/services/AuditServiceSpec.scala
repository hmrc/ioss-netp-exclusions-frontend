...
class AuditServiceSpec extends SpecBase with MockitoSugar with BeforeAndAfterEach {
  ...
  private val mockFrontendConfig = mock[FrontendAppConfig]
  private val mockAuditConnector = mock[AuditConnector]
  private val auditService = new AuditService(mockFrontendConfig, mockAuditConnector)

  override protected def beforeEach(): Unit = {
    reset(mockAuditConnector)
    when(mockFrontendConfig.appName).thenReturn("ioss-netp-exclusions-frontend")
  }
  ...
  "AuditService" - {
    ...
    "build and send an ExtendedDataEvent with the correct details" in {
      ...
      capturedEvent.auditSource mustBe "ioss-netp-exclusions-frontend"
